...
module.exports = function(srf, logger) {
  ...
  async function retrieveApplication(req, res, next) {
    ...
    const uri = parseUri(req.uri);
    const ctxarr = /context-(.*)/.exec(uri?.user);
    let lkeys = Object.keys(req.locals)
    logger.debug(`LOCALS: ${lkeys}`)
    try {
      let app;
      if (!app && req.locals.queue_name) {
        logger.debug(`calling to queue ${req.locals.queue_name}, generating queue app`);
        app = await createJambonzApp('queue', {account_sid, name: req.locals.queue_name});
      } 
      if (!app && req.locals.called_user) {
        logger.debug(`calling to registered user ${req.locals.called_user}, generating dial app`);
        app = await createJambonzApp('user',
          {account_sid, name: req.locals.called_user, caller_id: req.locals.callingNumber});
      }
      if (!app && req.locals.originatingUser) {
        const arr = /^(.*)@(.*)/.exec(req.locals.originatingUser);
        if (arr) {
          const sipRealm = arr[2];
          const clientDb = await registrar.query(req.locals.originatingUser);
          if (clientDb?.allow_direct_app_calling){
            logger.debug({app}, `searching all carriers for number`)
            let voip_carriers = await lookupAllVoipCarriers()
            for (const c of voip_carriers) {
              app = await lookupAppByPhoneNumber(req.locals.calledNumber, c.voip_carrier_sid);
            }
            if (app) logger.debug(`retrieved app ${app}`);
          }
          else {
            logger.debug(`looking for calling app for realm ${sipRealm}`);
      ...
      if (!app) {
        ...
        if (app) { logger.debug({app}, `retrieved  app for voip_carrier ${voip_carrier_sid}}`);}
      ...
      if (!app) {
          ...
          if (app) { logger.debug({app}, `retrieved device calling app by regex}`);}
      }
      //Check for assigned application_sid
      if (!app && req.locals.application_sid) {
            app = await lookupAppBySid(req.locals.application_sid);
      } 
      // Check for Application Header
      if (!app && req.has('X-Application-Sid')) {
            const application_sid = req.get('X-Application-Sid');
            app = await lookupAppBySid(application_sid);
      }
      // this is a transfer from another feature server
      if (!app && ctxarr) {
        const {retrieveKey, deleteKey} = srf.locals.dbHelpers;
        try {
          const obj = JSON.parse(await retrieveKey(ctxarr[1]));
          ...
          logger.error(err, `Error retrieving transferred call app for ${ctxarr[1]}`);
        }
      }
