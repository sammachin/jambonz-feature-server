  ...
  async function retrieveApplication(req, res, next) {
    ...
    try {
      ...
      if (req.locals.queue_name) {
        ...
        if (JAMBONZ_CLIENT_CALL_APP_SID) {
          logger.debug(`calling to registered user ${req.locals.called_user}, using CLIENT CALL APP`);
          app = await lookupAppBySid(JAMBONZ_CLIENT_CALL_APP_SID);
        } else {
          logger.debug(`calling to registered user ${req.locals.called_user}, generating dial app`);
          app = createJambonzApp('user', {account_sid, name: req.locals.called_user, caller_id: req.locals.callingNumber})
        }
        ...
        if (arr) {
          ...
          if (!app) {
          logger.info('Looking up by number')
          voip_carriers = await lookupAllVoipCarriers()
          for (const c of voip_carriers) {
              if (c.name == '_Clients'){
                logger.info(`Using carrier ${c.voip_carrier_sid}`)
                app = await lookupAppByPhoneNumber(req.locals.calledNumber, c.voip_carrier_sid);
              }
          }
          if (app){
            logger.info('retrieved app by phone number', {app})
          }
          }
          if (!app) {
            logger.info('Looking up by regex')
            voip_carriers = await lookupAllVoipCarriers()
            for (const c of voip_carriers) {
              if (c.name == '_Clients'){
                logger.info(`Using carrier ${c.voip_carrier_sid}`)
                app = await lookupAppByRegex(req.locals.calledNumber, c.voip_carrier_sid);
              }
            }
          }
          if (app){
            logger.info('retrieved app by regex ', {app})
          }
        }
      }
        ...
        if (arr) {
          ...
          logger.info(`VOIP Carrier SID: ${voip_carrier_sid}`)
          app = await lookupAppByPhoneNumber(req.locals.calledNumber, voip_carrier_sid);

          if (!app) {
            /* lookup by call_routes.regex */
            app = await lookupAppByRegex(req.locals.calledNumber, voip_carrier_sid);
          }
        }
      }
