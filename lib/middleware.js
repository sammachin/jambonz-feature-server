      ...
      if (req.locals.queue_name) {
        ...
        if (arr) {
          ...
          for (const c of voip_carriers) {
              if (c.name == '_Clients'){
                client_carrier == c.voip_carrier_sid
                logger.info(`Using carrier ${c.voip_carrier_sid}`)
                app = await lookupAppByPhoneNumber(req.locals.calledNumber, client_carrier);
              }
          }
          if (app){
          ...
          if (!app) {
            ...
            logger.info(client_carrier)
            app = await lookupAppByRegex(req.locals.calledNumber, client_carrier);
          }
          if (app){
